...
class TaylorShiftAttention(nn.Module):
    def __init__(self, dim, num_heads=8, head_dim=None, qkv_bias=True, proj_drop=0., attn_drop=0., a=.5, b=1., c=1.,
                 normalize_input=True, output_normalized=False):
        super().__init__()
        ...
        self.scale = self.head_dim ** 0.25 if normalize_input else 1.
        self.normalize_input = normalize_input
        self.output_normalized = output_normalized
        assert a > 0 and c >= b ** 2 / (4 * a), f"Choose a, b, c, such that ax^2+bx+c >= 0 for all x => " \
    ...
    def _efficient_attention(self, q, k, v):
        ...
        y = self.a * box_tensor(q, q) @ kv_mod \
...
class TaylorShiftBlock(Block):
    def __init__(self, dim, num_heads, qkv_bias=False, attn_drop=0., proj_drop=0., a=.5, b=1., c=1., normalize_input=True,
                 output_normalized=False, **kwargs):
        super().__init__(dim, num_heads, proj_drop=proj_drop, attn_drop=attn_drop, qkv_bias=qkv_bias, **kwargs)
        self.attn = TaylorShiftAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                         proj_drop=proj_drop, normalize_input=normalize_input,
...
class TaylorShiftViT(TimmViT):
    def __init__(self, img_size=224, a=.5, b=1., c=1., normalize_input=True, output_normalized=True,
                 conv_embed=False, patch_size=16, in_chans=3, embed_dim=768, conv_norm=nn.SyncBatchNorm, **kwargs):
        super().__init__(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                         block_fn=partial(TaylorShiftBlock, a=a, b=b, c=c, normalize_input=normalize_input,
                                          output_normalized=output_normalized),
                         **kwargs)
        ...
        if conv_embed:
            ...
            self.patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                              embed_dim=embed_dim, conv_norm=conv_norm)
...
class TextConvEmbed(nn.Module):
    def __init__(self, input_dim, embed_dim=768, bias=False, conv_norm=lambda channels: nn.GroupNorm(1, channels), depth=3):
        super().__init__()
        self.proj = nn.Sequential(*list_flatten([[nn.Conv1d(input_dim if i == 0 else embed_dim // 2**(depth-i),
                                               embed_dim // 2**(depth-i-1), 3, padding=1, bias=bias),
                                     conv_norm(embed_dim // 2**(depth-i-1)),
                                     nn.GELU()] for i in range(depth)]))
...
class TaylorShiftTransformer(TaylorShiftViT):
    def __init__(self, max_seq_len, input_dim, conv_embed=False, normalize_input=True, **kwargs):
        ...
        self.patch_embed = TextConvEmbed(input_dim, self.embed_dim) if conv_embed else nn.Linear(input_dim, self.embed_dim)

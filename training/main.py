...
_EXPNAMES = [
    ...
    "vit_fpga",
    "dense_trafo",
    "Distillation with Transformers",
    "BiAtt",
    "better_imagenet",
    "test",
]
...
def base_parser():
    ...
    group.add_argument(
        ...
        nargs="?",
        choices=["acc", "mIoU"],
        default=default_kwargs["perf_metric"],
        help="Performance metric to use for evaluation.",
    )
    ...
    group.add_argument(
        ...
        nargs="?",
        type=int,
        default=default_kwargs["num_heads"],
        help="Number of parallel attention heads.",
    )
    group.add_argument("-no_compile_model", action="store_true", help="Don't use torch.compile")
    group.add_argument(
        "-input_dim", nargs="?", type=int, default=default_kwargs["input_dim"], help="Dimensionality of text encoding."
    )
    group.add_argument(
        "-max_seq_len",
        nargs="?",
        type=int,
        default=default_kwargs["max_seq_len"],
    ...
    group.add_argument("-debug", action="store_true", help="Debug mode: lots of intermediate prints.")
    group.add_argument("-new_log", action="store_true", help="Create a new log file.")
    ...
    group.add_argument("-no_cuda", action="store_true", help="Don't use cuda.")
...
def check_arg_completeness(args, parser):
    ...
    if (
        ("dataset" not in args or args["dataset"] is None)
        and not inside_slurm()
        and args["task"] in ["pre-train", "fine-tune"]
    ):
        ...
        print(f"Using default dataset {args['dataset']} for task {args['task']}")
...
def create_runscript(args, file_name="runscript.tmp"):
    ...
    for key, val in slurm_defaults.items():
        if key not in args and key.replace("-", "_") not in args:
            args[key] = val
    task_args = ""
        ...
        if key.replace("_", "-") in slurm_defaults:
            ...
            if key == "job-name" and val is None:
                ...
                if "dataset" in args and args["dataset"] is not None:
                    ...
                    dataset = ""
                val = f'"{task} {model}{dataset}"'
            if key == "job-name" and not val.startswith('"'):
                val = f'"{val}"'
            if key in ["task-prolog", "nodes"] and val is None:
                continue

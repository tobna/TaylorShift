...
def mIoU(output, target, num_classes, ignore_index=-100):
    ...
    with torch.inference_mode():
        ...
        miou_val = iou.nanmean().item()
    return miou_val


def accuracy(output, target, topk=1, dict_key="acc{}", ignore_index=-100):
...
def calculate_metrics(
    args, model, rank=0, input=None, device=None, did_training=False, world_size=1, all_metrics=True, n_ims=1
):
...
def macs(args, model, input, n_ims=1):
    ...
    if n_ims is not None:
        input = input[:n_ims]
    with torch.cuda.amp.autocast() if args.eval_amp and args.cuda else nullcontext():
...
def throughput(args, model, input, device, iters=100):
    ...
    while True:
        ...
        if args.cuda:
            ...
            logging.debug(f"used {_get_ram_usage()} MiB of {_get_ram_total()} MiB RAM")
        if len(results) > 1 and (tp < 0.98 * results[-1][1] or tp <= 0.95 * max(res_tp for _, res_tp in results)):
            n_decr += 1
        else:
            n_decr = 0
        logging.debug(f"decreasing trend for {n_decr} steps in a row")
        results.append((bs, tp))
        logging.info(f"throughput calculation: throughput @ {bs} = {tp} images/second")
        if not args.cuda and _get_ram_usage() > 0.5 * _get_ram_total():
            logging.info(
                f"throughput calculation: used more than 50% of total RAM @ {bs}; stopping further calculation"
            )
            break
        if n_decr >= 2:
            logging.info(
                f"decreasing throughput trend for 3 sizes: {' -> '.join([f'{tp_r:.2f} @ {bs_r}' for bs_r, tp_r in results[-3:]])}; stopping further calculation"
            )
            break
        if len(results) >= 2 and results[-1][1] < 0.5 * results[-2][1]:
            logging.info(f"throughput calculation: throughput dropped below 50% of previous value @ {bs}; stopping now")
            break
        input = torch.cat((input, input), dim=0)
        bs *= 2
...
def _get_ram_usage():
    """Returns the RAM usage of this process in MiB.

    Returns
    -------
    int
        RAM usage of this process in MiB.
    """
    return resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024


def _get_ram_total():
    """Returns the total RAM of this system in MiB.

    Returns
    -------
    int
        Total RAM of this system in MiB.
    """

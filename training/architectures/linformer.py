...
class LinformerSelfAttention(nn.Module):
    ...
    def resize(self, seq_len):
        ...
        proj_k = self.proj_k.permute(1, 0).view(
            1, self.k, self.seq_len
        )  # 1 x k x seq_len (old)
        proj_k_tokens, proj_k = proj_k[:, :, :extra_tokens], proj_k[
            :, :, extra_tokens:
        ].view(1, self.k, old_imsize, old_imsize)
        ...
        if not self.share_kv:
            ...
            proj_v_tokens, proj_v = proj_v[:, :, :extra_tokens], proj_v[
                :, :, extra_tokens:
            ].view(1, self.k, old_imsize, old_imsize)
    ...
    def forward(self, x, context=None, **kwargs):
        ...
        assert (
            kv_len == self.seq_len
        ), f"the sequence length of the key / values is set to be {self.seq_len}, but is {kv_len}"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attn = LinformerSelfAttention(
            dim, seq_len, k=k, heads=num_heads, dropout=attn_drop
        )


class Linformer(Transformer):
    def __init__(self, *args, k=256, max_seq_len, **kwargs):
        # max_seq_len has to be the exact sequence length
        # include the [CLS] token
        super().__init__(
            *args,
            block_fn=partial(LinformerBlock, seq_len=max_seq_len, k=k),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            act_layer=act_layer,
            block_fn=partial(
                LinformerBlock, seq_len=num_patches + num_prefix_tokens, k=k
            ),
        )
...
def linformer_vit_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    if "layer_scale_init_values" in kwargs:
        kwargs["init_values"] = (
            kwargs["layer_scale_init_values"]
            if "layer_scale" in kwargs and kwargs["layer_scale"]
            else None
        )
    sizes = vit_sizes["Ti"]
    model = LinformerViT(
...
@register_model
def linformer_lra(input_dim, max_seq_len, **kwargs):
...
@register_model
def linformer_lra_imdb(input_dim, max_seq_len, **kwargs):
...
@register_model
def linformer_lra_cifar(input_dim, max_seq_len, **kwargs):

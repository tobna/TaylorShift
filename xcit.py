...
def conv3x3(in_planes, out_planes, stride=1, conv_norm=nn.SyncBatchNorm):
    """3x3 convolution with padding"""
    return torch.nn.Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False), conv_norm(out_planes)
    )
...
class ConvPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, bias=False, conv_norm=nn.SyncBatchNorm):
        super().__init__()
        ...
        if patch_size[0] == 16:
            self.proj = torch.nn.Sequential(
                conv3x3(in_chans, embed_dim // 8, 2, conv_norm=conv_norm),
                nn.GELU(),
                conv3x3(embed_dim // 8, embed_dim // 4, 2, conv_norm=conv_norm),
                nn.GELU(),
                conv3x3(embed_dim // 4, embed_dim // 2, 2, conv_norm=conv_norm),
                nn.GELU(),
                conv3x3(embed_dim // 2, embed_dim, 2, conv_norm=conv_norm),
            )
        elif patch_size[0] == 8:
            self.proj = torch.nn.Sequential(
                conv3x3(in_chans, embed_dim // 4, 2, conv_norm=conv_norm),
                nn.GELU(),
                conv3x3(embed_dim // 4, embed_dim // 2, 2, conv_norm=conv_norm),
                nn.GELU(),
                conv3x3(embed_dim // 2, embed_dim, 2, conv_norm=conv_norm),
...
class LPI(nn.Module):
    ...
    def __init__(
        ...
        self.act = act_layer()
        self.bn = norm_layer(in_features)
...
def xcit_small_12_p16(
    ...
    **kwargs
    ...
    size = dict(patch_size=16, embed_dim=384, depth=12, num_heads=8, mlp_ratio=4)
    if "num_heads" in kwargs:
        size["num_heads"] = kwargs["num_heads"]
    model = XCiT(
        **size,
        qkv_bias=qkv_bias,
        img_size=img_size,
